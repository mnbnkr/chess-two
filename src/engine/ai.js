...
export function chooseAiAction(state, color = "black", options = {}) {
  ...
  settings.softDeadline =
    settings.timeLimitMs > 0
      ? settings.startedAt + settings.timeLimitMs
...
function searchRoot(state, color, settings, actions, depth) {
  ...
  for (const action of actions) {
    ...
    if (
      score > bestScore ||
      (score === bestScore &&
        compareAiActions(state, action, bestAction, color) < 0)
    ) {
      bestScore = score;
      bestAction = action;
    }
...
export function evaluateState(state, color = "black") {
  ...
  score +=
    (state.currentPlayer === color ? 1 : -1) *
    Math.min(currentActions.length, 20) *
    2;
...
function isBadFatalShieldBreak(state, action) {
  ...
  if (!actor || !pathEffectReport(state, action).diesAfterAction) return false;
...
function actionHeuristic(
  ...
  if (destination)
    score +=
      actorSign *
      squareQuality(destination.r, destination.c, actorPerspective) *
      5;
  if (action.from && destination) {
    ...
    score +=
      actorSign * lifeDeathGateMoveBonus(state, action, actorPerspective);
    score +=
      actorSign * lifeDeathMoveActionValue(state, action, actorPerspective);
...
function defensiveActionOrderingScore(state, action, color, context) {
  ...
  const actorOwner = actor
    ? ownerOf(actor)
    : (action.color ?? state.currentPlayer);
  ...
  if (action.mode === "heal" && targetRisk > 0 && targetOwner === actorOwner) {
    score +=
      sign *
      Math.min(600, targetRisk * 0.7 + shieldValueForType(action.target?.type));
  }
...
function shieldTradeDiscipline(actor, action) {
  if (!action.target?.hadShield || action.target?.type === PIECE_TYPES.KING)
    return 0;
  const actorStake =
    materialValue(actor.type) +
    (actor.hasShield ? shieldValueForType(actor.type) : 0);
...
function shieldBreakTacticalBonus(after, actor, action, color) {
  ...
  let bonus =
    90 + targetShield * 1.45 + targetBase * 0.16 + cheapness * 0.55 + pawnLever;
...
function defensiveRootScore(before, after, actor, action, color) {
  ...
  const afterRisk = pieceExposureRisk(
    after,
    afterActor.id,
    ownerOf(afterActor),
  );
...
function selfDestructionPenalty(after, actor, action, color) {
  if (!actor || ownerOf(actor) !== color) return 0;
  if (
    after.gameOver?.winner === color ||
    action.target?.type === PIECE_TYPES.KING
  )
    return 0;
  if (findPieceById(after, action.pieceId)) return 0;

  const stake = pieceStake(actor);
  let penalty = 420 + stake * 1.55;
  if (action.deathStaging || action.deathLanding) penalty += 680;
...
function postActionExposurePenalty(state, action, color) {
  ...
  const exposureWeight =
    immediateGain >= worstReply ? 0.35 : action.target?.hadShield ? 1.1 : 0.85;
...
function pawnMoveQuality(state, action, color) {
  ...
  const overextensionPenalty =
    step === 3 && fileQuality <= 2.5 && !isGate ? 38 : 0;
  const centralAdvance = fileQuality * (step === 1 ? 9 : 14);
  const blockedCenterBonus = fromHome && fileQuality >= 3.5 ? 18 : 0;
  return (
    centralAdvance + blockedCenterBonus - edgePenalty - overextensionPenalty
  );
}
...
function positionalValue(piece, state, color) {
  ...
  if (piece.type === PIECE_TYPES.PAWN) {
    ...
    if (
      !isLifeDeathGateFile(piece.col) &&
      centerFileValue(piece.col) <= 1.5 &&
      Math.abs(piece.row - (piece.color === COLORS.BLACK ? 1 : 8)) >= 2
    ) {
      value -= 55;
    }
  }
...
function shieldPressureValue(target) {
  return (
    26 +
    shieldValueForType(target?.type) * 1.08 +
    materialValue(target?.type) * 0.1
  );
}
...
function lifeDeathPositionValue(piece) {
  ...
  const ownHalfDepth =
    owner === COLORS.BLACK ? piece.row : BOARD_SIZE - 1 - piece.row;
...
function lifeDeathMoveActionValue(state, action, color) {
  ...
  const mobilityDelta =
    lifeDeathMobilityFromSquare(state, piece, action.to, action) -
    lifeDeathMobilityFromSquare(state, piece, action.from, action);
  const centerDelta =
    centerFileValue(action.to.c) - centerFileValue(action.from.c);
  const tempo =
    state.turn.standardMoveMade && !state.turn.specialMoveMade ? 95 : 34;
  ...
  for (const dr of [-1, 1]) {
    for (const dc of [-1, 1]) {
      ...
      value +=
        sign *
        (110 +
          materialValue(target.type) * 0.34 +
          (target.hasShield ? shieldValueForType(target.type) * 0.55 : 0));
    }
  }
  return value;
}
  ...
  for (const dr of [-1, 1]) {
    for (const dc of [-1, 1]) {
      ...
      value +=
        sign *
        (72 +
          shieldValueForType(target.type) * 0.95 +
          materialValue(target.type) * 0.08);
    }
  }
  return value;
}
...
function targetActionValue(action) {
  ...
  return (
    base +
    (action.target?.hadShield ? shieldValueForType(action.target.type) : 0)
  );
}
...
function pieceStake(piece) {
  ...
  return (
    ...
    (piece.hasShield ? shieldValueForType(piece.type) : 0)
  );
}
...
function healActionValue(state, action, color) {
  const targetValue = materialValue(action.target?.type);
  const value =
    150 + shieldValueForType(action.target?.type) * 2 + targetValue * 0.1;
...
function lifeDeathTransferScore(state, action, color) {
  ...
  if (
    action.target?.type === PIECE_TYPES.KING &&
    ownerFromSnapshot(action.target) !== beforeOwner
  )
    return 120_000;

  const specialValue = materialValue(action.pieceType);
  const handoffPenalty =
    specialValue * (action.mode === "kill" ? 7.2 : 6.2) +
...
function lifeDeathAnnihilationDoomed(state, action) {
  ...
  for (const [dr, dc] of [
    ...
    if (
      !neighbor ||
      neighbor.id === actor.id ||
      !isLifeDeathType(neighbor.type)
    )
      continue;
...
function lifeDeathGateMoveBonus(state, action, color) {
  ...
  const adjacentSpecial =
    action.from.c === 1
      ...
      : getPiece(
          state.board,
          color === COLORS.BLACK ? 0 : BOARD_SIZE - 1,
          BOARD_SIZE - 1,
        );
  const opensSpecialPiece =
    adjacentSpecial &&
    (adjacentSpecial.type === PIECE_TYPES.LIFE ||
      adjacentSpecial.type === PIECE_TYPES.DEATH) &&
...
function lifeDeathAccessScore(state, color) {
  return (
    sideLifeDeathAccess(state, color) -
    sideLifeDeathAccess(state, oppositeColor(color)) * 0.95
  );
}
...
function sideLifeDeathAccess(state, color) {
  ...
  for (const piece of allPieces(state)) {
    if (
      ...
      continue;
    const ownHalfDepth =
      color === COLORS.BLACK ? piece.row : BOARD_SIZE - 1 - piece.row;
...
function kingSafetyScore(state, color) {
  ...
  if (enemyKing)
    score -=
      nearbyAlliedProtection(state, enemyKing, oppositeColor(color)) * 12;
  return score;
}
...
function shieldRepairContextValue(piece, alliedLifeCount) {
  ...
  return (
    (18 +
      Math.min(2, alliedLifeCount) * 16 +
      shieldValueForType(piece.type) * 0.18) *
    typeWeight *
    shieldNeedWeight
  );
}
...
function canBeHealedByOwner(state, target, healerOwner) {
  ...
  for (const life of allPieces(state)) {
    if (life.type !== PIECE_TYPES.LIFE || ownerOf(life) !== healerOwner)
      continue;
    if (
      Math.abs(life.row - target.row) === 1 &&
      Math.abs(life.col - target.col) === 1
    )
      return true;
  }
  return false;
...
function actionExposureValue(action) {
  ...
  if (action.kind === "attack") {
    if (action.target?.hadShield) {
      ...
      return (
        shieldPressureValue(action.target) + cheapAttackerLeverage + pawnLever
      );
    }
    return base * 1.04 + 80;
  }
...
function stateKey(state, depth, color) {
  ...
  return [
    ...
    state.enPassant?.crossed
      ?.map((square) => `${square.r},${square.c}`)
      .join(";") ?? "",
    pieces,
  ].join("~");
}
...
function shouldStartDepth(settings, depth, lastDepthMs, maxDepth) {
  ...
  const depthGrowth =
    depth >= 5 ? settings.depthStartMargin * 1.35 : settings.depthStartMargin;
